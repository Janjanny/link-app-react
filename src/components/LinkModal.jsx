  ...
  return (
    <div className='fixed top-0 left-0 w-screen h-screen flex items-center justify-center z-50'>
      ...
      <div className='relative z-10 px-[2.5rem] py-[3rem] bg-black-card text-white rounded-md shadow'>
        ...
        <input type="text" name="price" id="price" className="block rounded-md border-0 py-1.5 pl-3  ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 text-black w-[20rem]" placeholder="Enter link" value={url} onChange={(e) => {setUrl(e.target.value)}}></input>

        <label  className="block text-sm font-medium leading-6 text-gray-900 mt-8">Category</label>
        <select  className="block w-full rounded-md border-0 py-1.5 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:max-w-xs sm:text-sm sm:leading-6 text-black pl-3" value={category} onChange={(e) => {setCategory(e.target.value)}}>
